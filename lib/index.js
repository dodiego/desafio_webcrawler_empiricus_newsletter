...
async function fetchArticles (browser, page, result = []) {
  ...
  for (let article of articles) {
    ...
    if (moment().diff(articleDate, 'days') <= MAX_DAYS_INTERVAL) {
      ...
      await articlePage.addScriptTag({ url: 'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js' })
      await articlePage.evaluate(() => $('html, body').animate({ scrollTop: document.body.scrollHeight }, 'slow'))
      await articlePage.waitForFunction(() => document.querySelectorAll('img[class*="b-lazy"][data-src]').length === 0,
        { timeout: 2 * 60 * 1000 })
