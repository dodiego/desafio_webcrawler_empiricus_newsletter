...
const ARTICLE_BATCH_TIMEOUT = 2 * 60 * 1000
moment.locale('pt-BR')
...
async function fetchArticles (browser, page, result = []) {
  ...
  await Promise.all(articles.map(async (article) => {
    ...
    if (daysDifference <= MAX_DAYS_DIFFERENCE) {
      ...
      await articlePage.goto(article.url, {timeout: ARTICLE_BATCH_TIMEOUT})
  ...
  if (fetchNextPage) {
    ...
    await page.goto(nextPageUrl, {timeout: ARTICLE_BATCH_TIMEOUT})
